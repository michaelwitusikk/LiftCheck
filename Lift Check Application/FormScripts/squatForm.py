    ...
    with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
        while cap.isOpened():
            ...
            try:
                ...
                cv2.putText(image, "Left Knee: " + str(left_knee_angle), (15, 35), cv2.FONT_HERSHEY_SIMPLEX, .75, (0, 55, 255), 2)
                cv2.putText(image, "Left Hip Angle: " + str(left_hip_angle), (15, 60), cv2.FONT_HERSHEY_SIMPLEX, .75, (0, 55, 255), 2)
                    ...
                    if current_rep:
                        ...
                        if (left_knee_angle - 15 > current_rep.min_angle):
                            if current_rep.min_angle >= 90:
                                current_rep.add_form_issue("Not to depth (90 degrees or below)")
                                thread = Thread(target = say_issue, args = ('./parallelissue.mp3',))
                                thread.start()
                            rep_count += 1
                            #end = time.perf_counter()
                            #current_rep.timeup = end-start
                            #print(current_rep.timeup)
